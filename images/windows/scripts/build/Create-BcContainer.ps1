...
# Derive files-only image tag (if not already suffixed)
$filesOnlyImageName = if ($genericImageName.ToLower().EndsWith('-filesonly')) { $genericImageName } else { "$genericImageName-filesonly" }
Write-Host "[BC CACHE] Files-only image determined: $filesOnlyImageName"

Invoke-Section -Name 'Pull generic BC images' -Action {
    ...
    docker pull $filesOnlyImageName
    if ($LASTEXITCODE -ne 0) { throw "Failed to pull files-only BC image $filesOnlyImageName (exit $LASTEXITCODE)" }
}
...
# Backwards compatibility junction (old dash path -> new dot path)
if (-not (Test-Path 'C:\bcartifacts-cache') -and ($cacheDir -eq 'C:\bcartifacts.cache')) {
    try {
        New-Item -ItemType Junction -Path 'C:\bcartifacts-cache' -Target $cacheDir -ErrorAction SilentlyContinue | Out-Null
    } catch { Write-Host "[BC CACHE] Warning: Could not create legacy junction: $_" }
}
...
Invoke-Section -Name 'Write metadata' -Action {
    $metadata = [ordered]@{
        ...
        filesOnlyImage  = $filesOnlyImageName
